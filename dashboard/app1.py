...
with st.sidebar:
    ...
    selected = option_menu(
        ...
        ["Dashboard", "Suspicious Posts", "Analytics","Working"],
        icons=["speedometer2", "exclamation-triangle", "graph-up","gear"],
        default_index=0
    )

    st.markdown("---")
    st.markdown("### Last Update")
    st.write(pd.Timestamp.now().strftime("%H:%M:%S"))
...
if selected == "Working":
    ...
    st.markdown(
    """
    This system detects fake news using a **multimodal AI pipeline** combining
    **text analysis (BERT)** and **metadata analysis (RandomForest)**.
    """
    )

    st.divider()

    st.subheader("AI Detection Pipeline")
    ...
    st.success("This pipeline runs continuously to detect misinformation in real time.")